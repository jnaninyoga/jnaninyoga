...
export default function Map({title}){
    ...
    return(
        <section ref={map} className="py-6 mb-12 h-fit sm:min-w-[1150px] flex w-screen flex-1 justify-start items-center flex-col gap-4 bg-yoga-white texture-v sm:texture-h texture-opacity-80 overflow-hidden">
        ...
        <iframe
        className={`${isMapIntersected ? "opacity-100" : "opacity-0"} border-none w-full sm:max-h-[500px] max-h-[500px] object-cover object-center aspect-video transition-all duration-700`}
        title={title}
