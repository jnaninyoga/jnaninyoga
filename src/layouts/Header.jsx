...
export default function Header() {
  ...
  return (
    <header role="menu" className={`fixed ${hideNavbar ? '-top-[100%] opacity-0' : 'top-0 opacity-100'} z-[9999] bg-yoga-white sm:px-10 px-4 h-14 sm:h-16 w-full flex justify-between items-center transition-all duration-300`}>
      ...
      <nav role="menubar" className={`bg-yoga-white lg:pt-0 sm:pt-0 pt-10 lg:h-full h-screen lg:w-auto w-screen lg:relative fixed z-[9999] top-0 flex items-center sm:justify-center justify-start gap-6 lg:flex-row flex-col texture-v lg:bg-none lg:before:bg-none lg:right-auto lg:left-auto ltr:lg:right-auto ltr:lg:left-auto rtl:lg:right-auto rtl:lg:left-auto transition-all ${isMenuHidden ? "ltr:-right-[120%] rtl:-left-[120%]" : "ltr:right-0 rtl:left-0"}`}>
        ...
        <ul role="navigation" className="flex items-center lg:gap-4 sm:gap-8 gap-4 lg:flex-row flex-col z-[40]">
          {
            Tnavbar().map((link, index) => (
              <li key={index} role="link" onClick={hideMenu}>
