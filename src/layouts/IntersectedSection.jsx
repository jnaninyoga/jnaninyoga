...
export default function IntersectedSection({title, text, video, alt="", reverse=false}) {
  ...
  return (
    <section className={`w-full h-[500px] flex flex-1 items-center justify-center flex-col ${reverse ? "sm:flex-row-reverse" : "sm:flex-row"}`} id={title}>
        <video ref={vsec} onContextMenu={e => e.preventDefault()} src={video} muted loop autoPlay preload='auto' className={`${isVsecIntersected  ? "translate-x-[0%] opacity-100 shadow-yoga-green": `${reverse ? "translate-x-[100%]":"translate-x-[-100%]"} opacity-0 shadow-none`} sm:w-1/2 w-full sm:h-full h-[300px] max-h-[500px] max-w-screen-lg ${reverse ? "sm:mr-14":"sm:ml-14"} object-cover object-center aspect-square transition-all duration-1000 select-none pointer-events-none`} alt={alt} ></video>
