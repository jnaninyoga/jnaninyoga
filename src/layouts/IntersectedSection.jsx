...
export default function IntersectedSection({title, text, video, alt="", poster="", reverse=false}) {
  ...
  return (
    <section dir='ltr' className={`w-full h-[500px] flex flex-1 items-center justify-center flex-col ${reverse ? "sm:flex-row-reverse" : "sm:flex-row"}`} id={title}>
        ...
        <article dir={currentLanguage().dir} ref={psec} className={`${isPsecIntersected ? `${reverse ? "sm:translate-x-14":"sm:-translate-x-14"} opacity-100 translate-x-0` : `${reverse ? "translate-x-[-100%]":"translate-x-[100%]"} opacity-0`} h-full sm:px-10 px-8 sm:py-12 py-8 sm:w-1/2 w-[92%] bg-yoga-white flex items-center ${currentLanguage().dir === "rtl" ? "sm:justify-end" : "sm:justify-start"} justify-center flex-col sm:gap-6 gap-4 sm:translate-y-0 -translate-y-24 transition-all duration-1000 overflow-hidden`}>
            <div className={`w-full flex items-center sm:justify-start justify-center sm:flex-row flex-col sm:gap-4 gap-1`}>
                ...
                <h2 className={`${isPsecIntersected ? "translate-y-0 opacity-100" : "translate-y-[100%] opacity-0"} text-2xl cinzel sm:text-start text-center font-bold delay-500 duration-500 transition-all`}>{title}</h2>
            </div>
            <p dir={currentLanguage().dir} className={`${isPsecIntersected ? "translate-y-0 opacity-100" : "translate-y-[100%] opacity-0"} sm:text-base text-justify delay-500 duration-500 transition-all`}>{text}</p>
        </article>
    </section>
  )
}