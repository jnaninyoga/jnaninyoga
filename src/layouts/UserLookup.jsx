	...
	return (
		<li title={title} className={` ${type == "bullet" ? "flex" : "block"} pb-1`} >
			...
			{link ? (
				...
				<span className={`${styledText && "cinzel"} ${type !== "bullet" && "leading-relaxed"} pl-2`}>
					{value}
				</span>
			)}
		</li>
...
export default function UserLookup({ user }) {
	...
	return (
		<>
			...
			<section className="relative h-[90%] w-[95%] md:w-[780px] md:h-[700px] flex flex-col gap-2 bg-yoga-white print:bg-white sm:overflow-x-hidden overflow-y-auto bg-texture texture-v-1 before:opacity-20 print:absolute print:left-0 print:top-0 print:h-screen print:w-screen">
