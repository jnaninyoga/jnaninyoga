...
function Bullet({type="bullet", title, label, value, link, icon=false, styledText=false, sm=true, underline=false}){
    return (
    <li className={` ${type == "bullet" ? "flex" : "block"} pb-1`} title={title}>
        <label tabIndex={1} className={`cinzel relative outline-none group ${sm && "text-sm"} font-semibold uppercase text-yoga-green-dark ${type == "bullet" ? "flex items-center gap-1" : "inline-block"} ${ label && `before:transition-all before:absolute before:h-[2px] ${underline ? "before:w-full" : "before:w-0"} before:bg-yoga-green-dark before:left-0 before:-bottom-[2px] hover:before:w-full hover:before:bg-yoga-green focus:before:w-full focus:before:bg-yoga-green`}`}>
            ...
            { label && <span className={`cinzel text-yoga-green-dark group-hover:text-yoga-green group-focus:text-yoga-green transition-all`}>{label}</span> }
        </label>
        { link ?
            <a href={link} className={`${styledText && "cinzel"} pl-2 outline-none hover:text-yoga-green focus:text-yoga-green hover:underline focus:underline underline-offset-4 transition-all`} target="_blank" rel="noreferrer">{value}</a>
            :
            <span className={`${styledText && "cinzel"} ${type !== "bullet" && "leading-relaxed"} pl-2`}>{value}</span>
        }
    </li>
    );
}
    ...
    <section className="relative h-[90%] w-[95%] md:w-[720px] md:h-[858px] flex flex-col gap-2 bg-yoga-white print:bg-white sm:overflow-x-hidden overflow-y-auto bg-texture texture-v-1 before:opacity-20 print:absolute print:left-0 print:top-0 print:h-screen print:w-screen">
        <section className='relative h-full w-full min-w-max px-4 py-6 print:px-0 print:pr-5 print:py-4 flex items-center flex-col gap-8 print:gap-6'>
            ...
            <header className='w-full pl-5 flex justify-start items-center gap-4'>
                ...
                <div className='w-full min-w-max h-full flex flex-col justify-center gap-1'>
                    ...
                    <h1 className={`flex ${itsLongName ? "flex-col text-3xl" : "text-4xl"} cinzel text-3xl uppercase font-semibold`}>
                        { itsLongName ? 
                            <><span className='cinzel'>{user.firstname}</span><span className='cinzel'>{user.lastname}</span></>
                            :
                    ...
                    <div className='w-full flex flex-col'>
                        <ul className='w-full flex gap-4'>
                            <Bullet styledText title={`${user.firstname} ${user.lastname}, Birth date`} value={dateFormater(user.birthdate, false)} icon="fi fi-ss-cake-birthday" />
                            <Bullet styledText value={user.sex} icon={"fi fi-br-couple"}/>
                            <Bullet styledText title={`${user.age} Years old`} value={`${user.age} Years old`} icon={"fi fi-ss-user"}/>
                        </ul>
                        <ul className='w-full flex'>
                            <Bullet styledText title={`registred on ${dateFormater(user.createdAt)}`} value={dateFormater(user.createdAt)} icon={"fi fi-sr-user-time"}/>
                        </ul>
                    </div>
                </div>
            </header>
            ...
            <article className='w-full min-w-max flex flex-col gap-1 px-10'>
                <div className='w-full flex gap-24'>
                    ...
                    <ul className='flex flex-col'>
                        <Bullet styledText title={`${user.firstname} ${user.lastname}, Phone`} value={user.phone} icon="fi fi-ss-smartphone" link={`tel:${user.phone}`} />
                        <Bullet styledText title={`${user.firstname} ${user.lastname}, WhatsApp Number`} value={user.phone} icon="fi fi-brands-whatsapp" link={whatsappLink(user.phone)} />
                    </ul>
                    {/* professional informations */}
                    <ul className='flex flex-col'>
                        <Bullet styledText title={`${user.firstname} ${user.lastname}, Email`} value={user.email} icon="fi fi-bs-at" link={`mailto:${user.email}`} />
                        <Bullet styledText title={`${user.firstname} ${user.lastname}, Job`} value={user.job} icon="fi fi-ss-briefcase" />
                    </ul>
                </div>
                <div className='' title={`${user.address}`}><i className="fi fi-ss-map-marker text-yoga-green-dark mr-2"></i><a href={`https://www.google.com/maps/search/?api=1&query=${user.address}`} className="hover:text-yoga-green hover:underline underline-offset-4 transition-all" target="_blank" rel="noreferrer">{user.address}</a></div>
            </article>
            ...
            <article className='w-full min-w-max flex flex-col gap-4 pl-10'>
                ...
                <ul className='w-full flex flex-col'>
                    <Bullet styledText sm={false} underline title={`${user.firstname} ${user.lastname}, Medical record`} label="Medical Record:" value={user.medicalhistory.record} />
                    <Bullet type='stack' sm={false} underline title={`${user.firstname} ${user.lastname}, Medical record informations`} label="Medical Record Informations:" value={user.medicalhistory.recordinfo || "No Additional Informations"} />
                </ul>
            </article>
            ...
            <article className='w-full min-w-max flex gap-1 pl-10'>
                ...
                <ul className='w-full flex flex-col'>
                    <Bullet styledText sm={false} title={`${user.firstname} ${user.lastname}, Sport`} label="Sport:" value={user.liferhythm.sport} icon="fi fi-ss-gym" />
                    <Bullet styledText sm={false} title={`${user.firstname} ${user.lastname}, Meditation`} label="Meditation:" value={user.liferhythm.meditation} icon="fi fi-sr-leaf" />
                </ul>
            </article>
